...
const EditRecipe = () => {
  ...
  useEffect(() => {
    const fetchRecipeData = async () => {
      try {
        setLoading(true);
        const response = await fetch(`https://recipeappapi.azurewebsites.net/api/recipe/${id}`, { //https://localhost:7005/api/Recipe${id}
          method: 'GET',
          headers: {
  ...
  const handleSaveIngredient = async (ingredient) => {
    try {
      ...
      let url = 'https://recipeappapi.azurewebsites.net/api/Ingredient'; //https://localhost:7005/api/Ingredient
      let method = 'POST';
      let body = {
        recipeId: id,
  ...
  const handleRemoveIngredient = async (ingredient) => {
    try {
      ...
      if (!ingredient.new) {
        ...
        const response = await fetch(`https://recipeappapi.azurewebsites.net/api/Ingredient/${ingredient.id}`, { //https://localhost:7005/api/Ingredient/${ingredient.id}
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${authUser.token}`,
            'Accept': 'application/json',
...
// keywords
  ...
  const handleRemoveKeyword = async (keyword) => {
    try {
      ...
      const response = await fetch(`https://recipeappapi.azurewebsites.net/api/Recipe/${id}/Keywords?keyword=${keyword.word}`, { //https://localhost:7005/api/Recipe/${id}/Keywords?keyword=${keyword.word}
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${authUser.token}`,
          'Accept': 'application/json',
...
//images
  ...
  const handleSaveImages = async () => {
    try {
      ...
      for (const image of recipeImages) {
        let url = 'https://recipeappapi.azurewebsites.net/api/Image'; //https://localhost:7005/api/Image
        let method = 'POST';
        let body = {
  ...
  const handleRemoveImg = async (index) => {
    try {
      ...
      const response = await fetch(`https://recipeappapi.azurewebsites.net/api/Image/${index.id}`, { //https://localhost:7005/api/Image/${index.id}
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${authUser.token}`,
          'Accept': 'application/json',
