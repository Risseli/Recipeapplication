...
const Profile = () => {
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      const response = await fetch(`https://recipeappapi.azurewebsites.net/api/Review/${editingReviewId}`, {  // https://localhost:7005/api/Review/${editingReviewId}
        method: 'PUT',
        headers: {
  ...
  useEffect(() => {
    const checkUserStatus = async () => {
      try {
        ...
        const response = await fetch(`https://recipeappapi.azurewebsites.net/api/user/${authUser.userId}`, { // https://localhost:7005/api/user/${authUser.userId}
          method: 'GET',
          headers: {
            Authorization: `Bearer ${authUser.token}`,
          },
        ...
        if (data.admin) {
          ...
          const usersResponse = await fetch("https://recipeappapi.azurewebsites.net/api/User", { // https://localhost:7005/api/User
            method: "GET",
            headers: {
              Authorization: `Bearer ${authUser.token}`,
            },
...
const loadRecipes = async (option, id) => {
  try {
    ...
    if (option === "ownRecipes") {
      ...
      apiUrl = `https://recipeappapi.azurewebsites.net/api/User/${id}/Recipes`; // https://localhost:7005/api/User/${id}/Recipes
    } else if (option === "favorites") {
      // Hae käyttäjän suosikkireseptit
      apiUrl = `https://recipeappapi.azurewebsites.net/api/User/${id}/Favorites`; // https://localhost:7005/api/User/${id}/Favorites
    } else if (option === "reviews") {
      // Hae käyttäjän arvostelut
      apiUrl = `https://recipeappapi.azurewebsites.net/api/User/${id}/Reviews`; // https://localhost:7005/api/User/${id}/Reviews
    }
 ...
 const namesPromises = recipesData.map(async (recipe) => {
  const nameResponse = await fetch(`https://recipeappapi.azurewebsites.net/api/Recipe/${recipe.recipeId}`, { // https://localhost:7005/api/Recipe/${recipe.recipeId}
    method:'GET',
  });
  ...
  const handleSaveClick = async () => {
    try {
      ...
      const response = await fetch(`https://recipeappapi.azurewebsites.net/api/user/${editedUser.id}`, { // https://localhost:7005/api/user/${editedUser.id}
      method:'GET',
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleDeleteClick = async () => {
    if (window.confirm(`Are you sure you want to delete profile "${user.name}"?`)) {
      try {
        const response = await fetch(`https://recipeappapi.azurewebsites.net/api/user/${user.id}`, { // https://localhost:7005/api/user/${user.id}
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${authUser.token}`, // Lisää token otsikkoon
          },
  ...
  const deleteReview = async (reviewId) => {
    if (window.confirm(`Are you sure you want to delete this review?`)) {
      try {
        const response = await fetch(`https://recipeappapi.azurewebsites.net/api/Review/${reviewId}`, { // https://localhost:7005/api/Review/${reviewId}
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${authUser.token}`, // Lisää token otsikkoon
          },
  ...
  const handleSaveAdminChanges = async () => {
    try {
      ...
      const updatePromises = users.map(async (user) => {
        await fetch(`https://recipeappapi.azurewebsites.net/api/User/${user.id}`, { // https://localhost:7005/api/User/${user.id}
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${authUser.token}`, // Lisää token otsikkoon
  ...
  const handleDeleteUser = async (userId) => {
    if (window.confirm(`Are you sure you want to delete this user?`)) {
      try {
        ...
        const response = await fetch(`https://recipeappapi.azurewebsites.net/api/User/${userId}`, { // https://localhost:7005/api/User/${userId}
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${authUser.token}`, // Lisää token otsikkoon
          },
